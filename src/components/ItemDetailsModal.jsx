  ...
  return (
    <>
      ...
      {isFullscreen && (
        ...
        >
          <button className="absolute right-6 top-6 text-3xl font-bold text-white transition hover:text-slate-300">✕</button>
          <img 
            src={mainImage} 
            alt="Full size item" 
          ...
          <div className="md:w-1/2 bg-slate-100 p-4 flex flex-col gap-4 overflow-y-auto">
            ...
            >
              <img 
                src={mainImage} 
                alt="Main item" 
                className="w-full h-64 md:h-80 object-cover rounded-xl border border-slate-200 transition group-hover:opacity-90" 
              />
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center opacity-0 transition group-hover:opacity-100">
                <span className="rounded-full bg-slate-900/60 px-4 py-2 text-sm font-medium text-white backdrop-blur-sm shadow-lg">
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {allImages.map((img, idx) => (
                  <img 
                    ...
                    className={`h-20 w-20 shrink-0 object-cover rounded-lg cursor-pointer border-2 transition ${mainImage === img ? 'border-emerald-500 opacity-100' : 'border-transparent opacity-60 hover:opacity-100'}`} 
                  />
                ))}
              </div>
            )}
